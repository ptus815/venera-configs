...
async function getResponse(url, header, request, hasRetried) {
	try {
		...
		if ((response.status < 200 || response.status > 400 || response.body === null) && hasRetried === false) {
			return await getResponse(url, header, request, true)
		} else {
			return response
		}
	} catch (erro) {
	...
	try {
		if (path !== "/") {
			...
			for ( const [key, value] of request.headers.entries() ) {
				...
				if ( key.toLowerCase() === "host" || lowerKey.includes('cf-') || lowerKey.includes('x-') || lowerKey.includes('if-') || lowerKey.includes('cache-') ) {
					continue;
				}
				requestHeader.set(key, value);
			}

			const response = await getResponse(url, requestHeader, request, false)
			...
			for (const [key, value] of response.headers) {
				...
				if (lowerKey === "connection" || lowerKey.startsWith("cf")) {
					continue;
				}
		...
		return new Response(`错误：${erro.message}`, {
			status: 404
		});
	}
}
